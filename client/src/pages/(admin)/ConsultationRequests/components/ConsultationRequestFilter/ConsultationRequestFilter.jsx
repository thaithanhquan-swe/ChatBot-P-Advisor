...
function ConsultationRequestFilter({ filters, onChange, onApply, onReset }) {
  ...
  return (
    <aside className='mt-5 rounded-xl border border-slate-200 bg-white p-4'>
      ...
      <form
        onSubmit={onApply}
        className='grid grid-cols-1 items-end gap-3 md:grid-cols-2 xl:grid-cols-7'
      >
        ...
        <FilterLabel label='Trạng thái'>
          <select
            ...
            onChange={set('status')}
            className='h-9 w-full rounded-lg border border-slate-200 px-3 text-[10px]'
          >
        ...
        <FilterLabel label='Từ ngày'>
          <input
            ...
            className='h-9 w-full rounded-lg border border-slate-200 px-2 text-[10px]'
          />
        </FilterLabel>
        ...
        <FilterLabel label='Sắp xếp theo'>
          ...
          >
            ...
            <option value='resolvedAt'>Ngày hoàn thành</option>
          </select>
        </FilterLabel>
        <FilterLabel label='Thứ tự'>
          <select
            value={filters.sortDirection}
            onChange={set('sortDirection')}
            className='h-9 w-full rounded-lg border border-slate-200 px-3 text-[10px]'
          >
        ...
        <div className='flex gap-2'>
          <button className='h-9 flex-1 rounded-lg bg-[#D71920] px-3 text-[11px] font-semibold text-white'>
            Áp dụng
          </button>
          <button
            type='button'
