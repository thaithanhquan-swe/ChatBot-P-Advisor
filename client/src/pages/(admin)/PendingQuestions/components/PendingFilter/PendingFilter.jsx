...
function PendingFilter() {
  return (
    <aside className='mt-5 overflow-hidden rounded-xl border border-slate-200 bg-white p-4'>
      ...
      <FilterLabel label='Danh mục'>
        <Select text='Tất cả danh mục' />
      </FilterLabel>
      <FilterLabel label='Trạng thái'>
        <Select text='Tất cả trạng thái' />
      </FilterLabel>
      <FilterLabel label='Độ ưu tiên'>
  ...
  return (
    <div className='mt-4 border-t border-slate-100 pt-4'>
      ...
      <div className='flex items-center gap-3'>
        <div
          className='h-21 w-21 shrink-0 rounded-full p-4'
          style={{
            background:
              'conic-gradient(#d71920 0 44%, #ff9d20 44% 70%, #72a7e8 70% 86%, #cbd5e1 86% 100%)',
          }}
        >
...
function DateInput({ text }) {
  return (
    <div className='relative'>
      <input
        placeholder={text}
        className='h-9 w-full rounded-lg border border-slate-200 px-2 pr-6 text-[9px] outline-none'
      />
      <CalendarDays
        size={13}
