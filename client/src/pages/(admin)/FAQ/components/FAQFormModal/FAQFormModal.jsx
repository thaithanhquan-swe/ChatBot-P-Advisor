...
function FAQFormModal({ open, faq, categories, onClose, onSubmit }) {
  ...
  return (
    <ModalShell
      open={open}
      onClose={onClose}
      title={faq ? 'Chỉnh sửa FAQ' : 'Tạo FAQ mới'}
      description='Nhập đầy đủ câu hỏi, câu trả lời, danh mục và trạng thái.'
    >
      <form onSubmit={handleSubmit} className='space-y-5 p-6'>
        <Field label='Câu hỏi' required error={errors.question}>
          <textarea
            rows={3}
            value={form.question}
        ...
        <Field label='Câu trả lời' required error={errors.answer}>
          <textarea
            rows={7}
            value={form.answer}
            onChange={(e) => set('answer', e.target.value)}
            placeholder='Nhập nội dung câu trả lời...'
            className='w-full resize-y rounded-lg border border-slate-200 px-3 py-2.5 text-xs leading-5 outline-none focus:border-red-300'
          />
        </Field>
        ...
        <div className='flex justify-end gap-2 border-t border-slate-100 pt-5'>
          <button
            type='button'
            onClick={onClose}
            className='h-10 rounded-lg border border-slate-200 px-5 text-xs font-semibold text-slate-600 hover:bg-slate-50'
          >
            Hủy
          </button>
          <button
