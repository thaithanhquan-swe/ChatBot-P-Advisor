...
const FAQItem = ({ faq, isExpanded, onToggle, onAskInChat }) => {
  ...
  return (
    <article
      className={`overflow-hidden rounded-xl border bg-white transition-all duration-300 hover:border-red-200 ${
        isExpanded
          ? 'border-(--primary-color) shadow-[0_4px_15px_rgba(0,0,0,0.05)]'
          : 'border-gray-200'
      }`}
    >
      <button
        ...
        onClick={onToggle}
        className={`flex w-full cursor-pointer items-center justify-between gap-4 px-6 py-4.5 text-left text-[1.05rem] font-semibold transition-colors ${
